...
function LoginPage() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      // Store only safe user data and route to that user's role dashboard.
  ...
  return (
    <AuthLayout
      title="Welcome Back"
      subtitle="Login with your email and password. You will be redirected to your role dashboard."
    ...
    >
      ...
      <p className="mt-4 text-center text-xs text-slate-500">
        Backend routes used: <code>/api/auth/login</code> and role dashboard redirect.
      </p>

      <div className="mt-2 text-center text-xs text-slate-500">
        <Link className="underline hover:text-slate-700" to="/register">
          Need register first?
        </Link>
      </div>
    </AuthLayout>
